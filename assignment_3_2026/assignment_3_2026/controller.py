...
def rot_world_to_yaw_body(yaw: float) -> np.ndarray:
    """
    Rotation matrix from world frame to "yaw-only body" frame.
    This matches the interface expected by run_test.py telemetry conversion.
    """
    c = float(np.cos(yaw))
    s = float(np.sin(yaw))
    return np.array(
...
def get_integral_telemetry() -> dict:
    """
    External telemetry interface used by run_test.py (optional plot process).

    The plotting code expects these keys:
      - pid_i_body: (ix, iy, iz)
      - yaw_i_term: float
      - dob_comp_body: (dx, dy, dz)   (0 if not available)
      - vel_est_body: (vx, vy, vz)    (0 if not available)
      - vel_model_body: (vx, vy, vz)  (0 if not available)
    """
    # This controller integrates position error in the global frame; expose it as-is.
    # (The plot uses it qualitatively; keeping a stable interface matters more than frame purity.)
    pi = np.asarray(_state.get("pos_integral", np.zeros(3)), dtype=float).reshape(3)
    yaw_i_term = float(KI_YAW * float(_state.get("yaw_integral", 0.0)))

    dob = np.asarray(_state.get("disturbance_estimate", np.zeros(3)), dtype=float).reshape(3)
    # In this implementation, the disturbance estimate lives in the same frame as final_vel/vel_desired_body;
    # run_test converts body->world for XY, so treat it as "body" for compatibility.
    dob_comp_body = (-dob).astype(float)

    return {
        "pid_i_body": (float(pi[0]), float(pi[1]), float(pi[2])),
        "yaw_i_term": yaw_i_term,
        "dob_comp_body": (float(dob_comp_body[0]), float(dob_comp_body[1]), float(dob_comp_body[2])),
